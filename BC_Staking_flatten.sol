...
contract BillionCashStaking is Ownable {
    ...
    uint256 public minimumStakeValue = 1 * 10**16;

    uint256 public DAILY_ROI = 20;
    uint256 public REFERRAL_PERCENTS = 120;
    ...
    //sets the minimum stake value
    function setReferralPercents(uint256 _REFERRAL_PERCENTS)
        external
        onlyOwner
    {
        REFERRAL_PERCENTS = _REFERRAL_PERCENTS;
    }
    //sets the minimum stake value
    function setDailyROI(uint256 _DAILY_ROI)
        external
        onlyOwner
    {
        DAILY_ROI = _DAILY_ROI;
    }
}
